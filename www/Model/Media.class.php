...
class Media extends MysqlBuilder{
    ...
    public function setCreateMedia($categorie,$email,$action){
        $messages = [];
        if($_FILES['file']['error'] > 0) { 
            $messages[] = 'Une image est nécéssaire';
        } else {
    ...
    public function setEditMedia($categorie,$email,$action){
        $messages = [];
        if($_FILES['file']['error'] > 0) { 
            $messages = [];
        } else {
    ...
    public function setAllMedia($categorie,$email,$action){
        $messages = [];
        if (isset($_FILES)) {
            $target_dir = __DIR__ .'/../Style/images/'.$categorie.'/';
            if (!file_exists($target_dir) && !is_dir($target_dir))
