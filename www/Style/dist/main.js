...
function sliderInit(slider) {
  ...
  slider.html(container); // Ajouter la navigation

  var nav = $('<nav></nav>');
  nav.append('<button class="prev"></button>');
  nav.append('<button class="next"></button>');
  slider.append(nav); // ajouter un attribut data-currentSlide au slider
...
function slide(slider) {
  ...
  if (currentSlide == -1) {
    ...
    container.on('transitionend', function () {
      ...
      container.css('transition', 'none'); // Ramener le slider sur la dernière image
      ...
      slide(slider);
      setTimeout(function () {
        enableNav(slider); // Supprimer le clone situé au début du container

        container.find('.slide:first').remove(); // Rétablir la transition du container
  ...
  if (currentSlide == container.find('.slide').length) {
    ...
    container.on('transitionend', function () {
      ...
      container.css('transition', 'none'); // Ramener le slider sur la première image

      slider.attr('data-currentSlide', 0);
      slide(slider);
      setTimeout(function () {
        enableNav(slider); // Supprimer le clone situé à la fin du container

        container.find('.slide:last').remove(); // Rétablir la transition du container
